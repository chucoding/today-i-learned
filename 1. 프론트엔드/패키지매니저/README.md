...
### 의존성 격리
pnpm의 의존성 격리는 **각 패키지가 자기에게 필요한 의존성만 따로 보이게 한다**는 방식이다.

| 방식 | 매니저 | 동작 |
|------|--------|------|
| **Hoisting(승격)** | npm / Yarn (classic) | `npm install` / `yarn` 시 대부분의 의존성이 프로젝트 루트 `node_modules`로 올라감. 예: react가 필요하면 한 번만 루트에 설치되고, 앱·keycloakify·@rabyss/ds 모두 같은 `node_modules/react`를 참조. 구조가 단순하고 “React는 하나”가 자연스럽게 보장됨. |
| **격리 + 심링크** | pnpm | 의존성은 `.pnpm` 저장소에 패키지별·버전별로 쌓임. 각 패키지마다 자기 전용 `node_modules`가 있고, 그 안에는 실제로 쓰는 패키지만 심링크로 연결됨. 루트 `node_modules`에 보이는 것도 대부분 심링크이며, “누가 어떤 react를 쓰는지”가 경로로 구분됨. |

**의존성 격리** = 각 패키지가 자기 디렉터리 안에서만 의존성을 해석하고, 필요한 것만 심링크로 가져온다는 뜻이다.
...
node_modules/
  ...
  @rabyss/ds     → .pnpm/@rabyss+ds@1.0.1/.../node_modules/@rabyss/ds
  keycloakify    → .pnpm/keycloakify@11.9.6/.../node_modules/keycloakify
.pnpm/
  react@18.3.1/node_modules/react/                    ← 실제 파일
  @rabyss+ds@1.0.1_react@18.3.1/.../node_modules/
    react        → ../../react@18.3.1/node_modules/react  (심링크)
```
