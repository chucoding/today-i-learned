...
## 핵심
코드를 **수직(레이어)·수평(슬라이스)·기술 목적(세그먼트)** 로 나누어 재사용을 통제하고, 높은 응집도·낮은 결합도를 유지
...
## 레이어 (Layers)
자세한 내용은 [layer.md](./layer.md) 참고.

## Slices
자세한 내용은 [slice.md](./slice.md) 참고.

## Segments
슬라이스 내 기술적 목적별 구분
> 이름을 정할 때는 폴더 안에 무슨 파일이 들어 있는지가 아니라 무엇을 위해 존재하는지(목적)이 드러나도록 작성
> - components, hooks, modals, assets (x)
> - ui, model, api (o)
> - App, Shared는 slice 없이 segment로만 구성. 

| **세그먼트 (Segments)** | 슬라이스 내 기술적 목적별 구분 |  |


- **ui**: UI 컴포넌트, 포맷터, 스타일, 아이콘 등
  - 아이콘은 `import HcLogo from @../hc-logo.svg?react`와 같이 평탄화하여 컴포넌트로 사용 가능
  - `Icon.tsx`와 같은 공통 컴포넌트는 만들지 않는다.(프로젝트에 포함된 사용하지 않는 SVG까지 초기 번들에 포함될 가능성을 높이고, tree-shaking 적용이 안됨.)
...
- 예: enum은 용도에 따라 `api` 또는 `ui` 세그먼트의 사용처 근처에 정의한다.
- 타입 상세·enum 등은 FSD 공식 예시 문서 참고
